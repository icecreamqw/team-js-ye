...
# =============================================================================
def generate_multivariate_wave(length=1000, scale_tan=5.0):
    """Generate a synthetic multivariate time series using sin, cos, tan.

    Args:
        length (int): Number of time steps to generate.
        scale_tan (float): Scaling factor to reduce tan value amplitude.

    Returns:
        np.ndarray: Array of shape (length, 3) with sin, cos, scaled tan.
    """
    x = np.linspace(0, 20 * np.pi, length)
    sin = np.sin(x)
    cos = np.cos(x)
    tan = np.tan(x) / scale_tan  # to avoid extreme spikes
    data = np.stack([sin, cos, tan], axis=1)
    return data  # shape: (length, 3)